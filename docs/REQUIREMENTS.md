...
- **R21.6** Accepted, documented trade-off of the above: a stolen browser session authenticates for up to 30 days without contacting the provider again. Bounding that requires an **absolute session age** measured from `iat` — a deliberate policy, not a side effect of a token lifetime — and is deferred (R21.20).
...
- **R22.3** Where a client cannot report an actual cost, the same fallback ladder as R10.4 applies: reported tokens × catalog price, then estimated tokens × catalog price. **Unknown pricing degrades to zero here too** — and that is a deliberate hole, named so it is not mistaken for rigour: what an unpriced model *spends* goes unrecorded. The alternative, refusing to serve any model whose price is unknown, was rejected as disproportionate. This rule is about **recording**, and only recording: an *estimate* that cannot be priced is never treated as free, and a document scan that cannot be priced is refused outright (R22.21). The two fail in opposite directions — a recording that reads as zero under-counts, an estimate that reads as zero admits spending — so the cheap direction differs and the rules differ with it.
...
- **R22.20** During a **pricing outage** the cap counts four of six. Prices come from the OpenRouter catalog, so when it cannot be read every rung of R22.3's ladder below the provider's own reported cost returns zero: the four completion clients keep counting (their cost arrives with the response and needs no catalog) while the two embedding paths record nothing. This is R22.3's named hole rather than a separate one, the amounts are microdollars, and it is bounded by the outage. **Four of six is the best case, not the floor:** a completion response that arrives without a provider-reported cost also records zero during an outage, because every rung below that one needs the catalog. The number in this requirement is what a reader will plan around, so it is stated as a ceiling rather than a guarantee.
- **R22.21** An unknown price is **never treated as free**, which is a different question from R22.20. An *estimate* that cannot be priced is unmeasured, not zero, and falls to the flat fallback — except for a **document scan**, which is **refused outright** (R22.7): the fallback is turn-sized, a scan's fan-out is not, and flooring it admitted a 20 MB document (5,668 classifier calls, ~$2.44) against six cents of budget. The asymmetry is deliberate: a chat turn genuinely is one turn. The refusal is distinguishable from at-cap and from an unreachable ledger (R22.13), because a user who cannot upload during a pricing outage has not overspent.
