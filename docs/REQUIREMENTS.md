...
- **R19.5** GitHub tools ride the same tool-calling loop and dispatcher contracts as R16.1–R16.2: remote failures are returned to the model as error strings, never raised, and calls appear in the Last Tool Calls panel. A local tool name always shadows a remote one.
