...
- **R17.2** **Seeds in git, DB derived.** The source of truth is a folder of markdown seed files (`knowledgebase/*.md`), each with optional YAML frontmatter carrying a `category` and free-form `tags` (e.g. roles, seniority, countries); the files are versioned and code-reviewed. The runtime store is a single SQLite file under `data/` — a **derived, gitignored artifact** that any clone rebuilds on first run (this is also the reviewer/backup story: the repo alone is sufficient).
