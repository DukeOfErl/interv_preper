...
## 17. Provider data privacy

The app sends the user's resume and cover letter to a third party, so how that data may be used is a first-class user-facing concern, not an implementation detail. Privacy here is a **gate, not a notice**: when it cannot be established, the data does not leave. See ADR-0100 (the policy registry) and ADR-0110 (fail closed) for the rationale.

- **R17.1** **Enforce where the provider allows it.** Every outbound provider call carries whatever request parameters that provider offers for preventing training on, or retention of, submitted data. This applies to *all* call sites without exception — the interview stream, the per-turn chat guardrail, the document guardrail, the query condenser, the embeddings/indexing call, and the eval harness — because each one transmits the same user content.
- **R17.2** **Never silently overclaim.** Where a provider exposes no such parameter (OpenAI, Anthropic — neither trains on API traffic by default, but neither accepts a per-request instruction saying so), the app must not imply enforcement. The posture is reported as *stated by the provider*, with the date those terms were last verified.
- **R17.3** **Fail closed: unknown means blocked, not warned.** When the provider's posture cannot be established, user content **must not be transmitted at all** — no document upload, no message, no embedding call. Disclosure after egress is worthless, because an upload cannot be undone. The block is **not overridable**: there is no "send anyway" affordance. Passing the gate requires either an enforcing request parameter or the provider's published terms with a verified-on date; absence of a known-bad signal is not a basis for sending. (Extends the fail-closed principle of R15.6 from document screening to provider selection.)
- **R17.4** **The refusal happens before the user can act, and explains itself.** The blocked state is reached before any widget that accepts user content renders — the user must be unable to upload, not merely warned after uploading. The app states plainly that nothing was sent, and names how to register a known-private endpoint so a legitimate one is not a dead end.
- **R17.5** **Enforced by construction, not by a single check.** No object capable of transmitting user content to an unvouched-for provider may exist: the refusal is raised when the client is created, so a call site added later inherits the protection rather than having to remember it. A UI-level check alone is insufficient, since it protects only the paths that pass through it.
- **R17.6** **Always disclose, not only on failure.** The posture is shown on every run as a one-line, colour-coded status at the top of the Interview tab — above the document drop zone, so the user sees it before deciding what to upload. Three states, visually distinct: enforced (green), provider-stated (blue), not ensured (red). A fuller explanation is available but collapsed, so the status itself stays one line. The UI accessors must not raise, so a blocked run can still explain itself.
- **R17.7** **A privacy restriction may make a model unavailable, and that must be legible.** When no provider for the selected model satisfies the requirement, the request fails rather than routing to one that does not. The resulting error names the privacy restriction as a possible cause and points at changing the model, since retrying cannot help.
- **R17.8** **One source of truth.** The per-provider policy — its level, its request parameters, and its user-facing description — is defined in exactly one place (`privacy.py`), keyed by provider base URL, so what the app sends and what the UI claims cannot diverge. Adding a provider is data, not code.
- **R17.9** **Privacy enforcement must not depend on third-party library internals.** Where a library offers no documented way to attach the required parameters, the app owns the call rather than relying on undocumented forwarding behavior (hence the raw-SDK embedder). A test is not sufficient mitigation here: it catches the regression at CI time, but the window between a dependency upgrade and a test run is a window of silent, irreversible exposure.

---

# Planned scope

## 16. Web-sourced knowledge (planned)

- **R16.1** The app can search the internet for interview-relevant material: information about the target company, interviewing best practices, and technical interview questions for the target role.
- **R16.2** Fetched content is ingested through the **same pipeline as uploaded documents** (parse → screen → chunk → embed → index), tagged with type `web` and provenance (URL, fetch date). This shared pipeline is the primary design constraint §15 must satisfy.
- **R16.3** Web content is **untrusted by default**: guardrail screening (R15.6) is mandatory, not best-effort, before web text enters the index — a fetched page is an injection vector the user never even saw.
- **R16.4** Retrieved web chunks are distinguishable from user-document chunks in both the request context block and the dashboard, so the interviewer prompt can weight them differently (e.g. resume facts are authoritative about the candidate; web facts are background).
- **R16.5** Scope details (search provider, when searches trigger, result caps, caching) are deliberately unspecified until §15 is built.
