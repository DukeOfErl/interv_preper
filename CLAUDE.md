...
`.streamlit/secrets.toml` must also exist before the app serves anyone — it holds the `[auth]` OIDC config and the `[roles]` allowlist (see `.streamlit/secrets.toml.example`); `Authlib>=1.3.2` is a required dependency. The tests need neither: the identity port is injectable, so they pass an `Identity` directly and never perform a login (R21.16).
