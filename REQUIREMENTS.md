...
- **R15.7** On each turn, a query retrieves the **top-k chunks** from the index; k is a named constant. Retrieved chunks are formatted into a clearly labeled context block, each with provenance metadata (document name, type, chunk id), and injected per the contract in R15.8.
- **R15.16** **Query condensation.** On follow-up turns the retrieval query is not the raw message (an anaphoric fragment like "tell me more about that" retrieves poorly). A small, fast model rewrites the message — given recent conversation — into a **standalone search query**, using markdown instructions (behavior-in-markdown, `.ignore`-tagged like the guardrail). It **fails open**: any error or blank rewrite falls back to the raw message. It is **skipped on the first turn** (no referents to resolve) and whenever retrieval isn't happening (not grounding-aware, or no documents). The resolved query is shown in the developer dashboard's last-retrieval panel.
